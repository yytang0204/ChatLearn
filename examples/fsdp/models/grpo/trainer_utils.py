...
def bin_packing(seq_len_list, max_train_token):
    ...
    return bins_id, bins_seqlen

def bin_packing_fix_bin(seq_len_list, bin_size):
    """
    Implementation of best fit decreasing bin packing algorithm with fix bin
    """
    seqlen_id_mapping = {i: seq_len for i, seq_len in enumerate(seq_len_list)}
    sorted_mapping = dict(sorted(seqlen_id_mapping.items(), key=lambda item: item[1], reverse=True))
...
def regroup_data_packing(data_list, key_list, max_train_token):
    # data_b should contain all data in one minibatch
    data_b = data_list[0]
    regroup_data_list = []
    ...
    total_token = torch.tensor(sum([sum(seq_len) for seq_len in bin_seqlen])).cuda()
    torch.distributed.all_reduce(total_token, op=torch.distributed.ReduceOp.SUM)
    print(f"debugyy max_bin_in_dp: {bin_size}, bin_size: {len(bins_id)}, total token {total_token}")
    bins_id, bin_seqlen = bin_packing_fix_bin(seq_len_list=total_token_length, bin_size=bin_size.cpu().item())
    #print(f"debugyy max_bin_in_dp: {max_bin_in_dp}, bin_size: {bin_size}")
    print(f"debugyy bin_size: {len(bins_id)}, sample in each bins: {[len(bins) for bins in bins_id]}")
